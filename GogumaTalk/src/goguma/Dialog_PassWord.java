...
public class Dialog_PassWord extends JDialog implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==btnCheck){
			...
			else{ //this.setVisible(state);
				lblCheck.setText("OK!");
				lblCheck.setForeground(Color.GREEN);
					if(mtc.chatroom.size()>mtc.chatroom2.size()){
						mtc.pnl_c_c.removeAll();
						mtc.pnl_c_c.add(new PnlOpenChat(mtc,mtc.arrayMax,pnlNum,userId));
						mtc.pnl_c_c.revalidate();
						mtc.pnl_c_c.repaint();
						this.setVisible(false);
					}else{
						mtc.pnl_c_c.removeAll();
						mtc.pnl_c_c.add(mtc.chatroom2.get(pnlNum));
						mtc.pnl_c_c.revalidate();
						mtc.pnl_c_c.repaint();
						this.setVisible(false);
					}
