...
public class PnlProfile extends PnlSideBar implements ActionListener {
	...
	PnlProfile() {
		...
		pnl_North.setBackground(new Color(255, 255, 150));
		...
		lbSpace.setForeground(new Color(30,30,30,150));
	...
	static public void addPerson(String name, String connect){
		profilePerson.add(new PnlProfilePerson(name, connect));
		updatePerson();
	}
	...
	private static void updateFriendState() {
		...
		if(Main.isLogin==true){ // 전체 친구 중 온/오프라인 수 표시
			...
			for(int i=0; i<PnlProfile.profilePerson.size(); i++){
				if(PnlProfile.profilePerson.get(i).lbLastMsg.getText().equals("OnLine")){
					tempOnLine++;
				} else {
					tempOffLine++;
				}
			...
			lbSpace.setText(" Friend State : " + tempOnLine + " / " + PnlProfile.profilePerson.size());
